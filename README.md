...
```
#### 配置属性
id：字符串类型，Dom依赖唯一的名称id,用来生成canvas容器，必填

mode：字符串类型，设置渲染模式，必填

默认为‘auto’，进行图片大小等比例缩放适应canvas容器，
‘original’不进行图片缩放，加载原图片大小。
‘auto-rotate’ 旋转模式，支持图片旋转

imgUrl：字符串类型，设置图片的url访问地址，可以是http,https,cdn方式。如果需要图片强制刷新，可在图片url后增加时间戳，必填

event：对象类型，参见回调方法，选填
...
#### 配置属性
data：数组类型，跟setData实例方法数据一样，必填

isOpen：布尔类型，默认false为初始不开启绘画矩形，true为初始开启绘画矩形，选填
